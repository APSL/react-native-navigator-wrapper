...
class NavigatorWrapper extends React.Component {
  ...
  render () {
    ...
    let NavigationBar = <NavBar routeMapper={routeMapper} style={this.props.navBarStyle} />
    if (this.props.hideNavBar) {
      NavigationBar = null
    } else if (this.props.navBar) {
      NavigationBar = React.cloneElement(this.props.navBar, {routeMapper: routeMapper})
    }
    return (
      <Navigator
